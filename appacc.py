...
def rerank(query: str, chunks: list[dict], top_k=5):
    if not chunks:
        return []
    try:
...
def ask(req: AskRequest):
    ...
    candidates = hybrid_retrieve(req.question, q_vec, top_k=10)
    context_chunks = rerank(req.question, candidates, top_k=5)
